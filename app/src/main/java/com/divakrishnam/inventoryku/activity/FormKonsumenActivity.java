...
public class FormKonsumenActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_form_konsumen);
        ...
        konsumen = getIntent().getParcelableExtra(DATA);
        if(konsumen != null){
            etNama.setText(konsumen.getNamaKonsumen());
            etKontak.setText(konsumen.getKontakKonsumen());
            etAlamat.setText(konsumen.getAlamatKonsumen());
            Glide.with(getApplicationContext()).load(Urls.KONSUMEN_IMG_URL + konsumen.getGambarKonsumen()).error(R.drawable.ic_launcher_background).into(ivGambar);
        }
        ...
        EasyImage.handleActivityResult(requestCode, resultCode, data, this, new DefaultCallback() {
            ...
            public void onImagePicked(File imageFile, EasyImage.ImageSource source, int type) {
                CropImage.activity(Uri.fromFile(imageFile))
                        ...
                        .start(FormKonsumenActivity.this);
            }

            @Override
            public void onImagePickerError(Exception e, EasyImage.ImageSource source, int type) {
                super.onImagePickerError(e, source, type);
                Toast.makeText(FormKonsumenActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
    ...
    public void simpanData(Uri resultUri){
        ...
        if (resultUri != null){
            ...
            if (!nama.isEmpty() && !kontak.isEmpty() && !alamat.isEmpty() && !gambar.isEmpty()){
                ...
                if (konsumen != null){
                    RequestBody mKode = RequestBody.create(MediaType.parse("text/plain"), konsumen.getKodeKonsumen());
                    Call<ResponseInfo> call = apiService.putGambarKonsumen(mKode, mNama, mKontak, mAlamat, mGambar);
                    responseData(call, progressDialog);
                }else{
                    Call<ResponseInfo> call = apiService.postGambarKonsumen(mNama, mKontak, mAlamat, mGambar);
                    responseData(call, progressDialog);
                }
            }else{
            ...
            if (!nama.isEmpty() && !kontak.isEmpty() && !alamat.isEmpty()){
                if (konsumen != null){
                    Call<ResponseInfo> call = apiService.putKonsumen(konsumen.getKodeKonsumen(), nama, kontak, alamat);
                    responseData(call, progressDialog);
                }else{
                    Call<ResponseInfo> call = apiService.postKonsumen(nama, kontak, alamat);
                    responseData(call, progressDialog);
                }
            }else{
                progressDialog.dismiss();
